...
type FormData = {
  ...
  utmifyConfigId: string; successUrl: string; metaPixelId: string
...
function toForm(p?: Product | null): FormData {
  return {
    ...
    utmifyConfigId: p?.utmifyConfigId ?? '', successUrl: p?.successUrl ?? '',
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1.5">
        <button type="button" onClick={() => onChange('')}
          className={clsx('w-full flex items-center gap-2 px-3 py-2 rounded-lg border text-left text-xs transition-all',
            !selected ? 'bg-ep-accent/10 border-ep-accent text-ep-accent' : 'bg-ep-raised border-ep-border-default text-ep-secondary hover:border-ep-border-subtle')}>
          Nenhuma
        </button>
        {configs.map(c => (
          <button key={c.id} type="button" onClick={() => onChange(c.id)}
            className={clsx('w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border text-left text-xs transition-all',
              selected === c.id ? 'bg-ep-accent/10 border-ep-accent' : 'bg-ep-raised border-ep-border-default hover:border-ep-border-subtle',
              !c.enabled && 'opacity-50')}>
            <span className={clsx('font-medium', selected === c.id ? 'text-ep-accent' : 'text-ep-primary')}>{c.name || 'Sem nome'}</span>
...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  const sectionHasData = useCallback((id: string): boolean => {
    switch (id) {
      ...
      case 'avancado': return !!(form.successUrl || form.metaPixelId || form.customDomain || form.pixelIds.length > 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body = {
        ...
        utmifyConfigId: form.utmifyConfigId || null, successUrl: form.successUrl.trim(),
